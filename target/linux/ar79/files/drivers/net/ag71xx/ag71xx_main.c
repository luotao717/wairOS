...
#endif
	skb = dev_alloc_skb(AG71XX_RX_BUF_SIZE + 
			SKB_DATA_ALIGN(sizeof(struct skb_shared_info)) );
	if ( !skb )
	{
		return false;
...
/*	buf->rx_buf = data;*/
	...
	buf->dma_addr = dma_map_single(&ag->dev->dev, skb->data,
				       AG71XX_RX_BUF_SIZE, DMA_FROM_DEVICE);
...
static void ag71xx_hw_setup(struct ag71xx *ag)
{
	...
	{
		ag71xx_cb(ag, AG71XX_REG_MAC_CFG2,
			  		 MAC_CFG2_LEN_CHECK);
	}
...
/*		skb = build_skb(ring->buf[i].rx_buf, 0);*/
	...
	/*		kfree(ring->buf[i].rx_buf);*/
		...
		if (err) {
			...
			if ( ag71xx_get_pdata(ag)->switch_data)
			{
				err = rlvr_multi_skb_recv(skb,dev);
				if ( err )
				{
					dev->stats.rx_dropped++;
					printk(KERN_ALERT"receiv sdk on dev : %s for multi interface error\r\n",dev->name);
				}
			}
			else
	#endif /*CONFIG_AG71XX_ATHEROS_HEADER*/
...
static int __devinit ag71xx_probe(struct platform_device *pdev)
{
	...
	pr_info("device:%s %d probe .register the device driver\n",pdev->name,pdev->id);
	...
	if ( ag71xx_get_pdata(ag)->switch_data)
	{
		printk(KERN_ALERT"Init the sub interface on %s \r\n", dev->name);

		err =  rlvr_multi_global_init(dev);
		if ( err )
		{
			printk(KERN_ALERT"%s %d init the rlvr interface on %s error\r\n",
