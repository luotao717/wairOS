...
static int ag71xx_phy_connect_fixed(struct ag71xx *ag)
{
	...
	default:
		...
		pr_err("%s: invalid speed specified\n",dev->name);
		ret = -EINVAL;
		break;
	}

	/*netdev_dbg(dev, "using fixed link parameters\n");*/
	pr_debug("%s: using fixed link parameters\n",dev->name);	
...
static int ag71xx_phy_connect_multi(struct ag71xx *ag)
{
	...
	if (!phydev) {
		/*netdev_err(dev, "no PHY found with phy_mask=%08x\n",
			   pdata->phy_mask);*/
		pr_err("%s: no PHY found with phy_mask=%08x\n",dev->name,pdata->phy_mask);
		return -ENODEV;
	}
	...
	if (IS_ERR(ag->phy_dev)) {
		/*netdev_err(dev, "could not connect to PHY at %s\n",
			   dev_name(&phydev->dev));*/
		pr_err("%s: could not connect to PHY at %s\n",dev->name,dev_name(&phydev->dev));
	...
	pr_info("%s: connected to PHY at %s [uid=%08x, driver=%s]\n",
		    dev->name,dev_name(&phydev->dev), phydev->phy_id, phydev->drv->name);
...
int __devinit ag71xx_phy_connect(struct ag71xx *ag)
{
	...
	if (ag->mii_bus == NULL) {
		...
		pr_err("%s: unable to find MII bus on device '%s'\n",ag->dev->name,dev_name(pdata->mii_bus_dev));
		return -ENODEV;
	}
