...
#include <asm/mach-ar79/ath79_spi_platform.h>

#define DRV_DESC	"Atheros AR71xx/AR93xx SPI Controller driver"
...
static u32 ar71xx_spi_get_ioc_base(u8 chip_select, int cs_high, int is_on)
{
	...
	if (is_on == AR71XX_SPI_CS_INACTIVE)
		ret = AR71XX_SPI_IOC_CS_ALL; 
	else
		ret = AR71XX_SPI_IOC_CS_ALL & ~AR71XX_SPI_IOC_CS(chip_select);
...
static void ar71xx_spi_chipselect(struct spi_device *spi, int value)
{
	...
	case BITBANG_CS_INACTIVE:
		...
		__raw_writel(ioc_base, base + AR71XX_SPI_REG_IOC);
		break;
...
static void ar71xx_spi_setup_regs(struct spi_device *spi)
{
	...
	/* enable GPIO mode */
	ar71xx_spi_wr(sp, AR71XX_SPI_REG_FS, AR71XX_SPI_FS_GPIO);

	/* save CTRL register */
	sp->reg_ctrl = ar71xx_spi_rr(sp, AR71XX_SPI_REG_CTRL);

	/* TODO: setup speed? */
	ar71xx_spi_wr(sp, AR71XX_SPI_REG_CTRL, 0x43);
}
...
static void ar71xx_spi_restore_regs(struct spi_device *spi)
{
	...
	ar71xx_spi_wr(sp, AR71XX_SPI_REG_CTRL, sp->reg_ctrl);
	/* disable GPIO mode */
	ar71xx_spi_wr(sp, AR71XX_SPI_REG_FS, 0);
}
					...
					u32 word, u8 bits)
{
	...
	for (word <<= (32 - bits); likely(bits); bits--) {
		...
		__raw_writel(out, base + AR71XX_SPI_REG_IOC);

		__raw_writel(out | AR71XX_SPI_IOC_CLK, base + AR71XX_SPI_REG_IOC);

		word <<= 1;

#ifdef PER_BIT_READ
		/* sample MSB (from slave) on leading edge */
		ret = __raw_readl(base + AR71XX_SPI_REG_RDS);
		__raw_writel(out, base + AR71XX_SPI_REG_IOC);
#endif
