...
#include <asm/mach-ar79/irq.h> /* john add */
...
static char ath79_soc_type[ATH79_SYS_TYPE_LEN];
...
static void __init ath79_detect_sys_type(void)
{
	...
	if (soc_is_ar933x())
		sprintf(ath79_soc_type, "Atheros AR9330 (Hornet)");
	else if (soc_is_ar934x())
		sprintf(ath79_soc_type, "Atheros AR934x");
	else
		sprintf(ath79_soc_type, "Atheros AR934x emu");
}
...
/* john add for SDK wifi driver */
const char *get_soc_type(void)
{
	return ath79_soc_type;
}
EXPORT_SYMBOL(get_soc_type);
...
int get_wmac_irq(u_int16_t wmac_num)
{
	...
	if (strcmp(socType, "Atheros AR9330 (Hornet)") == 0)
		return ATH79_CPU_IRQ_IP2;
	else if (strcmp(socType, "Atheros AR934x") == 0)
		return ATH79_IP2_IRQ(1);
	else
		return ATH79_CPU_IRQ_IP2;
}
...
unsigned long get_wmac_base(u_int16_t wmac_num)
{
	...
	socType = get_soc_type();
	if (strcmp(socType, "Atheros AR9330 (Hornet)") == 0)
		return KSEG1ADDR(AR933X_WMAC_BASE);
	else if (strcmp(socType, "Atheros AR934x") == 0)
		return KSEG1ADDR(AR934X_WMAC_BASE);
	else 
		return KSEG1ADDR(AR913X_WMAC_BASE); /* need to be updated */
}
...
unsigned long get_wmac_mem_len(u_int16_t wmac_num)
{
	...
	socType = get_soc_type();
	if (strcmp(socType, "Atheros AR9330 (Hornet)") == 0)
		return AR933X_WMAC_SIZE;
	else if (strcmp(socType, "Atheros AR934x") == 0)
		return AR934X_WMAC_SIZE;
	else
		return AR913X_WMAC_SIZE;
}
