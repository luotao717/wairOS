...
static void OOB(webs_t wp, char_t *path, char_t *query)
{
    ...
    if(getAPMac("ra0", mac) != -1)
      	sprintf(SSID, "TOTOLINK_%s", mac);
    else
       	sprintf(SSID, "TOTOLINK_CARY");
	...
	if (WscStatus != 34 /* Configured*/){			// 34 == "Configured"
		...
		if (strcmp(nvram_bufget(RT2860_NVRAM, "WCNTest"), "1") == 0 ||
			...
			if (wsc_profile.Profile[0].EncrType == 0x0001) {
				...
				STF(0, "DefaultKeyID", "1"); //old value=2, modify by chenfei
				char temp[65];
				memset(temp, 0, 65);
...
static void WPSSTAPINStartEnr(char *bssid)
{
	...
	doSystem("iwpriv ra0 wsc_bssid %s\n", bssid);
...
static void WPSSTAPINStartReg(char *bssid, char *pin, char *force)
{
	...
	doSystem("iwpriv ra0 set WscForceSetAP=%s", force);
	...
	doSystem("iwpriv ra0 wsc_bssid \"%s\"", bssid);	
	doSystem("iwpriv ra0 wsc_start");
}
