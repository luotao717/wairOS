...
static int  getDdnsStatus(int eid, webs_t wp, int argc, char_t **argv);//by chenfei for ddns status
#if	defined (RT2860_APCLI_SUPPORT)
static int  getApcliStatus(int eid, webs_t wp, int argc, char_t **argv);//by chenfei for apcli status
#endif
...
static int  getCfg2ToHTML(int eid, webs_t wp, int argc, char_t **argv);
...
static void setLanguage(webs_t wp, char_t *path, char_t *query);//add by chenfei for support language
#if defined CONFIG_USER_STORAGE && (defined CONFIG_USB || defined CONFIG_MMC)
...
void formDefineUtilities(void)
{
	...
	websAspDefine(T("getDdnsStatus"), getDdnsStatus);//by chenfei for ddns status
#if	defined (RT2860_APCLI_SUPPORT)
	websAspDefine(T("getApcliStatus"), getApcliStatus);//by chenfei for apcli status
#endif
	...
	websAspDefine(T("getCfg2ToHTML"), getCfg2ToHTML);
	...
	websFormDefine(T("setLanguage"), setLanguage);//add by chenfei for support language
#if defined CONFIG_USER_STORAGE && (defined CONFIG_USB || defined CONFIG_MMC)
...
static int getDdnsStatus(int eid, webs_t wp, int argc, char_t **argv)
{
	int type;
	char_t *field;
	char *value;

	int enabled, ddns_status;
	char tmpBuf[100];
	struct stat st;
	...
	if ( !strncmp(field, T("DDNSStatus"),11) ) 
	{
		char *ddns_enabled = (char *)nvram_bufget(RT2860_NVRAM, "DDNSEnabled");
		enabled = atoi(ddns_enabled);

		if (!enabled)
		{
			sprintf(tmpBuf, "%s", "MM_ddns_disable");
		}
		else 
		{
			ddns_status = stat("/var/ddns_ok", &st);
			
			if (!ddns_status)
				sprintf(tmpBuf, "%s", "MM_ddns_success");
			else
				sprintf(tmpBuf, "%s", "MM_ddns_fail");
		}
		return websWrite(wp,T("%s"),tmpBuf);
	}
...
static int getLangBuilt(int eid, webs_t wp, int argc, char_t **argv)
{
#if defined (CONFIG_USER_GOAHEAD_LANG_EN_ZHCN)
	return websWrite(wp, T("0"));
#elif defined (CONFIG_USER_GOAHEAD_LANG_ZHCN)
	return websWrite(wp, T("1"));
#elif defined (CONFIG_USER_GOAHEAD_LANG_EN)
...
static int getApcliStatus(int eid, webs_t wp, int argc, char_t **argv)
{
	...
	if ( !strncmp(field, T("apcliStatus"),12) ) 
	{
		system("iwpriv apcli0 show connStatus");
		system("logread | grep \"ApCli0 Connected AP : \" > /tmp/apcliStatus");
		...
		if (p==NULL)//Not Found
			sprintf(tmpBuf, "%s", "MM_connect_success");
		else
			sprintf(tmpBuf, "%s", "MM_connect_fail");
...
static int getSysUptime(int eid, webs_t wp, int argc, char_t **argv)
{
	...
	if (utime->tm_hour > 0)
		return websWrite(wp, T("%d <script>dw(MM_hour)</script>%s, %d <script>dw(MM_min)</script>%s, %d <script>dw(MM_sec)</script>%s"),
				utime->tm_hour, (utime->tm_hour == 1)? "" : "",
				utime->tm_min, (utime->tm_min == 1)? "" : "",
				utime->tm_sec, (utime->tm_sec == 1)? "" : "");
	else if (utime->tm_min > 0)
		return websWrite(wp, T("%d <script>dw(MM_min)</script>%s, %d <script>dw(MM_sec)</script>%s"),
				utime->tm_min, (utime->tm_min == 1)? "" : "",
				utime->tm_sec, (utime->tm_sec == 1)? "" : "");
	else
		return websWrite(wp, T("%d <script>dw(MM_sec)</script>%s"),
				utime->tm_sec, (utime->tm_sec == 1)? "" : "");
	return 0;
}
...
static void setOpMode(webs_t wp, char_t *path, char_t *query)
{
	...
	if (!strncmp(old_mode, "0", 2)) {//bridge
	}
	else if (!strncmp(old_mode, "1", 2) || !strncmp(old_mode, "3", 2)) {//gateway | ap client
		if (!strncmp(mode, "0", 2)) {
#if defined CONFIG_RAETH_ROUTER || defined CONFIG_MAC_TO_MAC_MODE || defined CONFIG_RT_3052_ESW || defined CONFIG_ICPLUS_PHY
#else
			/*
...
static void setLanguage(webs_t wp, char_t *path, char_t *query)
{
	...
	langType = websGetVar(wp, T("langType"), T(""));

	//printf("##########\langType=%s\n", langType);

	nvram_set(RT2860_NVRAM, "LanguageType", langType);
	nvram_commit(RT2860_NVRAM);	
