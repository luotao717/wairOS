...
static void wirelessApcliBasic(webs_t wp, char_t *path, char_t *query);
...
void formDefineWireless(void) {
	...
	websFormDefine(T("wirelessApcliBasic"), wirelessApcliBasic);
	...
	if ((value == NULL) || (strcmp(value, "") == 0) ||
		...
		for (idx = 28; idx < 33; idx++)
			websWrite(wp, T("%s%d %s>%d%s%d%s"), "<option value=",
					...
					5180+20*idx+5, "MHz -- (", 36+4*idx+1, ")</option>\n");
...
static int getWlan11bChannels(int eid, webs_t wp, int argc, char_t **argv)
{
	...
	if ((value == NULL) || (strcmp(value, "") == 0) ||
		...
		/* ch14 */
		websWrite(wp, T("<option value=14 %s>2484MHz -- (14)</option>\n"), 
				(14 == channel)? "selected" : "");
	} else if (strcmp(value, "6") == 0) {
		/* ch3 ~ ch9 */
		for (idx = 2; idx < 9; idx++)
...
//add by chenfei for bridge change apclie to submit
static void wirelessApcliBasic(webs_t wp, char_t *path, char_t *query)
{
	char_t  *submitUrl = websGetVar(wp, T("submit-url"), T(""));

	char *apcli_enable = websGetVar(wp, T("apcliEnabled"), T(""));//add by chenfei

	if(!apcli_enable && !strlen(apcli_enable))
		return;

	if(atoi(apcli_enable)) {	// user choose nothing but press "apply" only
		nvram_set(RT2860_NVRAM, "apClient", "1");
	}
	else {
		nvram_set(RT2860_NVRAM, "apClient", "0");
	}
...
static void wirelessBridge(webs_t wp, char_t *path, char_t *query)
{
	...
	char_t	*apcli = websGetVar(wp, T("apcliEnbl"), T("0"));

	//new OperationMode
	if (!strncmp(bridgeMode, "1", 2)) //when bridge
	{
	...
	{	
		...
		{
			nvram_bufset(RT2860_NVRAM, "OperationMode", "3");	//when apclient(wisp)
			nvram_bufset(RT2860_NVRAM, "ApCliEnable", "1");
#if defined (RT2860_APCLI_SUPPORT)
...
void confWPAGeneral(int nvram, webs_t wp, int mbssid)
{
	...
	STFs(nvram, mbssid, "DefaultKeyID", "1");	//  //old value=2, modify by chenfei
...
inline void clearRadiusSetting(int nvram, int mbssid)
{
	...
	nvram_bufset(RT2860_NVRAM, racat("RADIUS_Key", mbssid+1), "totolink");
