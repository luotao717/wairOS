...
static void StorageAddUser(webs_t wp, char_t *path, char_t *query)
{
	...
	*/
	...
	if (0 != index) {
		...
		websHeader(wp);
		websWrite(wp, T("<h2><script>dw(JS_msg108)</script></h2><br>\n"));
		websFooter(wp);
		websDone(wp, 200);
	}
}
...
static int ShowSmbDir(int eid, webs_t wp, int argc, char_t **argv)
{
	...
	websWrite(wp, T("<td><script>dw(MM_all_users)</script></td>"));
	websWrite(wp, T("</tr>"));
	...
	while(EOF != fscanf(fp_mount, "%s %s %*s %*s %*s %*s\n", part, path))
	{
		...
		while(NULL != (dirp = readdir(dp)))
		{
			...
			if(S_ISDIR(statbuf.st_mode))
			{
				...
				{
					websWrite(wp, T("<tr><td><input type=\"radio\" name=\"dir_path\" value=\"%s/%s\"></td>"), path, dir_name);
