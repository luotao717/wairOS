...
//*******by chenfei for ssid and mac6 addr*****/
	...
	if (!strcmp(old_SSID, "TOTOLINK_CARY") || !strcmp(old_SSID, "TOTOLINK_7620F0"))
	{
		sprintf(new_SSID, "%s", "TOTOLINK_");
		if(getAP6Mac("ra0", mac6) != -1)
			strcat(new_SSID, mac6);

		//if (!strcmp(mac6, "7620F0") && !strcmp(new_SSID, "TOTOLINK_7620F0"))
		//{
		//	printf("test mac\n");
		//}
		//else
		{			
			...
			doSystem("iwpriv ra0 set SSID=%s", new_SSID);
		}
	}
...
static void setWan(webs_t wp, char_t *path, char_t *query)
{
...
#endif
	char_t  *static_mtu, *dhcp_mtu, *pppoe_mtu;
	const char	*opmode = nvram_bufget(RT2860_NVRAM, "OperationMode");
	...
	if (!strncmp(ctype, "STATIC", 7) || !strcmp(opmode, "0")) {
		...
		static_mtu = websGetVar(wp, T("staticMtu"), T("1500"));
		...
		nvram_bufset(RT2860_NVRAM, "wan_primary_dns", pd);
		nvram_bufset(RT2860_NVRAM, "wan_secondary_dns", sd);
		nvram_bufset(RT2860_NVRAM, "wan_static_mtu", static_mtu);
	}
	else {
		...
		if (!strncmp(ctype, "DHCP", 5)) {
			...
			dhcp_mtu = websGetVar(wp, T("dhcpMtu"), T("1500"));

			nvram_bufset(RT2860_NVRAM, "wanConnectionMode", ctype);
			nvram_bufset(RT2860_NVRAM, "wan_dhcp_hn", hostname);
			nvram_bufset(RT2860_NVRAM, "wan_dhcp_mtu", dhcp_mtu);
		}
			...
			pppoe_mtu = websGetVar(wp, T("pppoeMtu"), T("1492"));
			...
			nvram_bufset(RT2860_NVRAM, "wan_pppoe_mtu", pppoe_mtu);
		}
