 ...
 *
 *	$Id: //WIFI_SOC/release/SDK_4_1_0_0/source/user/goahead/src/firewall.c#1 $
 */
...
//add by chenfei for web url filter
static int  getWebURLEnableASP(int eid, webs_t wp, int argc, char_t **argv)
{
	int type, value;
	char *pfe = (char *)nvram_bufget(RT2860_NVRAM, "websURLFilterEnabled");
...
#if defined CONFIG_RALINKAPP_SUPERDMZ	//by chenfei for enable super dmz
...
static void makePortForwardRule(char *buf, int len, char *wan_ip, char *ip_address, int proto, int prf_int, int prt_int)
{
		int rc = 0;
		char *pos = buf;
//		getIfIp(getWanIfNamePPP(), wan_ip);
...
{
	char cmd[1024], *address, *tcpport80;
	...
	if(atoi(dmz_enable) == 1){
		...
		if(!address || !tcpport80){
			printf("Warning: can't find \"DMZAddress\" or \"DMZAvoidTCPPort80\" in flash\n");
			return;
		}

		makeDMZRule(cmd, sizeof(cmd), getWanIfNamePPP(), address, atoi(tcpport80));
		doSystem(cmd);
		...
		tcpport80 = (char *)nvram_bufget(RT2860_NVRAM, "DMZAvoidTCPPort80");
		snprintf(cmd, sizeof(cmd), "/bin/super_dmz %s %s %s", (tcpport80 && atoi(tcpport80)) ? "-t 80 " : "", portforward ? portforward : "", single_portforward ? single_portforward : "");
		doSystem(cmd);
	}
	return;
}
...
static int showIPPortFilterRulesASP(int eid, webs_t wp, int argc, char_t **argv)
{
	...
	while(getNthValueSafe(i, rules, ';', rec, sizeof(rec)) != -1 && strlen(rec)){
        ...
        //switch(atoi(action)){
        //    case ACTION_DROP:
		//		websWrite(wp, T("<td align=center id=portFilterActionDrop%d><script>dw(MM_drop)</script></td>"), i);
		//		break;
        //    case ACTION_ACCEPT:
		//		websWrite(wp, T("<td align=center id=portFilterActionAccept%d><script>dw(MM_accept)</script></td>"), i);
		//		break;
		//}
	...
	switch(atoi(default_policy)){
		case 0:
			websWrite(wp, T("<tr><td align=center colspan=6 id=portCurrentFilterDefaultAccept><script>dw(JS_msg109)</script></td><td align=center id=pktCnt%d>-</td></tr>"), i);
			break;
		case 1:
			websWrite(wp, T("<tr><td align=center colspan=6 id=portCurrentFilterDefaultDrop><script>dw(JS_msg109)</script></td><td align=center id=pktCnt%d>-</td></tr>"), i);
			break;
	}
...
static void ipportFilter(webs_t wp, char_t *path, char_t *query)
{
	...
	char *firewall_enable = (char *)nvram_bufget(RT2860_NVRAM, "IPPortFilterEnable");
...
//add by chenfei for portfoward
static void portForwardBasicSettings(webs_t wp, char_t *path, char_t *query)
{
	char_t  *submitUrl = websGetVar(wp, T("submit-url"), T(""));

	char *portforwad_enable = websGetVar(wp, T("forwardEnabled"), T(""));//add by chenfei

	if(!portforwad_enable && !strlen(portforwad_enable))
		return;

	if(atoi(portforwad_enable)) {	// user choose nothing but press "apply" only
...
static void portForward(webs_t wp, char_t *path, char_t *query)
{
	...
	char rule[8192];
	char *ip_address, *prf, *prt, *protocol, *comment;
...
static void BasicSettings(webs_t wp, char_t *path, char_t *query)
{
	...
	firewall_enable = websGetVar(wp, T("firewallEnabled"), T("0"));
...
static void DMZ(webs_t wp, char_t *path, char_t *query)
{
	...
	if((atoi(dmzE)==1) && !isIpValid(address))	// enable && invalid mac address
		return;
	if((atoi(dmzE)==2) && !isMacValid(address))	// enable && invalid mac address
		return;
...
#if 0
	char *wpfE = websGetVar(wp, T("pingFrmWANFilterEnabled"), T("0"));
	char *bpsE = websGetVar(wp, T("blockPortScanEnabled"), T("0"));
	char *bsfE = websGetVar(wp, T("blockSynFloodEnabled"), T("0"));
...
//	doSystem("iptables -t filter -F INPUT");
	...
	char_t	*wpfE, *bpsE, *bsfE, *spifw;
	char_t	*l2tp_pt, *pptp_pt, *ipsec_pt;

	wpfE = websGetVar(wp, T("pingFrmWANFilterEnabled"), T("0"));
	bpsE = websGetVar(wp, T("blockPortScanEnabled"), T("0"));
	bsfE = websGetVar(wp, T("blockSynFloodEnabled"), T("0"));
	spifw = websGetVar(wp, T("spiFWEnabled"), T("1"));

	l2tp_pt = websGetVar(wp, T("l2tpPT"), T("0"));
	pptp_pt = websGetVar(wp, T("pptpPT"), T("0"));
	ipsec_pt = websGetVar(wp, T("ipsecPT"), T("0"));
	
	nvram_bufset(RT2860_NVRAM, "WANPingFilter", wpfE);
	nvram_bufset(RT2860_NVRAM, "BlockPortScan", bpsE);
	nvram_bufset(RT2860_NVRAM, "BlockSynFlood", bsfE);
	nvram_bufset(RT2860_NVRAM, "SPIFWEnabled", spifw);
...
static void websSysRemote(webs_t wp, char_t *path, char_t *query)
{
	char_t  *submitUrl = websGetVar(wp, T("submit-url"), T(""));
...
static void websURLBasicSettings(webs_t wp, char_t *path, char_t *query)
{
	char_t  *submitUrl = websGetVar(wp, T("submit-url"), T(""));

	char *url_enable = websGetVar(wp, T("webURLEnabled"), T(""));//add by chenfei

	if(!url_enable && !strlen(url_enable))
		return;

	if(atoi(url_enable))// user choose nothing but press "apply" only
		nvram_set(RT2860_NVRAM, "websURLFilterEnabled", "1");
	else
		nvram_set(RT2860_NVRAM, "websURLFilterEnabled", "0");
...
static void websURLFilter(webs_t wp, char_t *path, char_t *query)
{
	...
	doSystem("iptables -t filter -F " WEB_FILTER_CHAIN);
	int url_enable = atoi(nvram_bufget(RT2860_NVRAM, "websURLFilterEnabled"));
	if(url_enable)
		iptablesWebsFilterRun();
...
static int getSuperDMZBuilt(int eid, webs_t wp, int argc, char_t **argv)
{
...
void formDefineFirewall(void)
{
	...
	websFormDefine(T("portForwardBasicSettings"), portForwardBasicSettings);//add by chenfei for port forward
	...
	websFormDefine(T("websURLBasicSettings"), websURLBasicSettings);//add by chenfei for web url
