...
/*add signal : sigqueue*/
static int nixio_sigqueue(lua_State *L) {
	int pid = luaL_checkint(L,1);
	int   sig = luaL_checkint(L,2);
	const char *val = luaL_checkstring(L, 3);
	union sigval sval;

	/*tmp ,set to 2 : signal from lua*/
	if ( val == NULL)
          {
/*            return nixio_perror(L);*/
  	      return luaL_argerror(L, 2, "supported values: nill string");
          }
        sval.sival_int=2;	
	
	return nixio__pstatus(L, !sigqueue(pid,sig,sval));
}
