...
function usb_info()
	local data={}
	local isshow=luci.util.execi("ls  /mnt")
	
	if not isshow then
		return 	
	end
	 
	for line in isshow do
		data[#data+1] = line
	end
...
function sysinfo()
	...
	local deviceOUI = tmpuci:get("devinfo","devinfo_0","ManufacturerOUI")
	local deviceSerial = tmpuci:get("devinfo","devinfo_0","SerialNumber")
	
	local model
	if type(deviceOUI) == "string" and type(deviceSerial) == "string" then		
		model = deviceOUI .. "-" .. deviceSerial
	else
		model =
...
function syslog()
	...
	local filepath = tmpuci:get("system","system_0","log_file")
	filepath = filepath or "/log/log.txt"
	return luci.util.exec("cat " .. filepath)
...
-- @param pid	Number containing the process id
-- @param sig	Signal to send (default: 15 [SIGTERM])
-- @param value messsage to send  ( string)
-- @return		Boolean indicating successful operation
-- @return		Number containing the error code if failed
process.sigqueue = nixio.sigqueue
...
function user.checkpasswd(username, pass)
	...
	if pwh and nixio.crypt(pass, pwh) ~= pwh then
		...
		--wxb add for forbin nil password or user
		if not pwh then
		    return false
		end
		return true
...
function wifi.getiwinfo(ifname)
	...
	if ifname then
		...
		elseif u:get("wireless", ifname) == "wifi_device" then
			u:foreach("wireless", "wifi_iface",
				function(s)
