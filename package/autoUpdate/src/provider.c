...
int ORANGELINK_update_entry(struct userInfo_cxy *info);
static struct service_cxy ORANGELINK_service = {
	ORANGELINK_NAME,
	ORANGELINK_update_entry,
	ORANGELINK_DEFAULT_SERVER,
	ORANGELINK_DEFAULT_PORT,
	ORANGELINK_REQUEST
};

int FANSUPBOUND_update_entry(struct userInfo_cxy *info);
static struct service_cxy FANSUPBOUND_service = {
	FANSUPBOUND_NAME,
	FANSUPBOUND_update_entry,
	FANSUPBOUND_DEFAULT_SERVER,
	FANSUPBOUND_DEFAULT_PORT,
	FANSUPBOUND_REQUEST
};
...
int FANSUPBOUND_update_entry(struct userInfo_cxy *info)
{
	...
	if(info->wildcard == 0)
	{
		len = sprintf(bp, "POST %s?Mac=%s&CmdType=upgrade&CmdId=%s&Status=%s&Msg=%s", "/fcp/router/excuteCmdResult",info->host,upcmdid,"1","success");
		bp += len;	
		len = sprintf(bp,	" HTTP/1.0\r\n"
	 						"Authorization: Basic %s\r\n"
	 						"User-Agent: %s\r\n"
	...
	{
		len = sprintf(bp, "POST %s?Mac=%s&CurrentSoftVersion=%s&CurrentHardwareVersion=%s", info->service->default_request,info->host,info->usrname,info->usrpwd);
		bp += len;	
		len = sprintf(bp,	" HTTP/1.0\r\n"
	 						"Authorization: Basic %s\r\n"
	 						"User-Agent: %s\r\n"
