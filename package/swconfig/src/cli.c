...
print_attr_val(const struct switch_attr *attr, const struct switch_val *val)
{
	...
	case SWITCH_TYPE_PORTS:
		...
		break;
	case SWITCH_TYPE_REGISTER:
		/*printf("the register %x  value %x \r\n",val->value.regrw->addr,val->value.regrw->data);	*/
		break;
	default:
		printf("?unknown-type?");
	}
...
/*	printf("swconfig dev <dev> [port <port>|vlan <vlan>] (help|set <key> <value>|get <key>|load <config>|show)\n");*/
	printf("\r\n"
		"swconfig dev <devname> <command>	-- switch config for dev \r\n"
		"<devname>		-- device name :eg eth1 \r\n"
		"<command>		-- operation command \r\n"
		"----------------- command description---------------------------\r\n"
		"	set <key> <value>	-- set the key value of global\r\n"
		"	get <key>		-- get the key value of global\r\n"
		"	load <config>		-- load the config of global\r\n"
		"	show			-- show the config of global\r\n"
		"	help			-- show usage\r\n"
		"	port <portnum> set <key> <value>	-- set the key value of the port\r\n"
		"	port <portnum> get <key>		-- get the key value of the port\r\n"
		"	port <portnum> load <config>		-- load the config of the port\r\n"
		"	port <portnum> show			-- show the config of the port\r\n"
		"	vlan <vlanid> set <key> <value>	-- set the key value of the vlan\r\n"
		"	vlan <vlanid> get <key>		-- get the key value of the vlan\r\n"
		"	vlan <vlanid> load <config>		-- load the config of the vlan\r\n"
		"	vlan <vlanid> show	-- show the config of the vlan\r\n"
		"	swreg  set <addr> <data> -- set the value of register \r\n"
		"	swreg  get <addr>	-- set the value of register \r\n"
		"	phyreg <id> set <addr> <data> -- set the value of the special phy gregister \r\n"
		"	phyreg <id> get <addr>	-- set the value of the special phy register \r\n"
		"-------------------------exsample----------------------------------------\r\n"
		"	value format,split with space: val1 val2 ...\r\n"
		"	1.set register of switch built in eth1 \r\n"
		"		swconfig dev eth1 reg set  <val_regaddr> <val_regdata>\r\n"	
		);
	exit(1);
}
...
int main(int argc, char **argv)
{
	...
	char *ckey = NULL;
	char *cvalue = NULL;
	char reg_key[]="switch_reg";
	char phy_reg_key[]="switch_phy_reg";
	int is_reg = 0;
	...
	for(i = 3; i < argc; i++)
	{
		...
		if (cmd != CMD_NONE) {
			...
			cvlan = atoi(argv[++i]);
		} else if (!strcmp(arg, "phyreg") && i+1 < argc) {
			cport = atoi(argv[++i]);
			is_reg = 2;
		} else if (!strcmp(arg, "swreg")) {
			is_reg = 1;
			...
			if ( is_reg >= 1 )
			{
				...
				cvalue = argv[++i];
				ptr = cvalue;
				/*int the for register */	
				regaddr = strtoul(ptr,&ptr,16); 
			}
			else
			{
				ckey = argv[++i];
