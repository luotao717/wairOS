...
static struct nla_policy register_policy[] = {
...
send_attr(struct nl_msg *msg, void *arg)
{
	...
	if ( regaddr > 0 )
	{
		NLA_PUT_U32(msg,SWITCH_ATTR_OP_REGISTER, regaddr);
	}
...
static int
store_register_val(struct nl_msg *msg, struct nlattr *nla, struct switch_val *val)
{
	struct nlattr *p;
	int err = 0;
	int remaining;
	struct switch_reg *regrw = NULL;

	val->value.regrw = malloc(sizeof(struct switch_reg));
	regrw = val->value.regrw;
	...
	nla_for_each_nested(p, nla, remaining) {
		...
		err = nla_parse_nested(tb, SWITCH_REG_ATTR_MAX, p, register_policy);
		if (err < 0)
			goto out;
		...
		regrw->addr = nla_get_u32(tb[SWITCH_REG_DATA]);
...
store_val(struct nl_msg *msg, void *arg)
{
	...
	else if (tb[SWITCH_ATTR_OP_VALUE_REGISTER])
		val->err = store_register_val(msg, tb[SWITCH_ATTR_OP_VALUE_REGISTER], val);
...
static int
send_attr_register(struct nl_msg *msg, struct switch_val *val)
{
	struct nlattr *n;
	int i;

	/* TODO implement multipart? */
	n = nla_nest_start(msg, SWITCH_ATTR_OP_VALUE_REGISTER);
	if (!n)
		goto nla_put_failure;
	{
		...
		np = nla_nest_start(msg, SWITCH_ATTR_REGISTER);
		if (!np)
			goto nla_put_failure;

		NLA_PUT_U32(msg, SWITCH_REG_ADDR, regrw->addr);
		NLA_PUT_U32(msg, SWITCH_REG_DATA, regrw->data);
...
send_attr_val(struct nl_msg *msg, void *arg)
{
	...
	case SWITCH_TYPE_REGISTER:
		if (send_attr_register(msg, val) < 0)
			goto nla_put_failure;
		break;
	default:
		goto nla_put_failure;
...
int swlib_set_attr_string(struct switch_dev *dev, struct switch_attr *a, int port_vlan, const char *str)
{
	...
	case SWITCH_TYPE_PORTS:
		...
		break;
	case SWITCH_TYPE_REGISTER:
		regrw = alloca(sizeof(struct switch_reg));
		memset(regrw, 0, sizeof(struct switch_reg));
		val.len = 0;
		ptr = (char *)str;
		while(ptr && *ptr)
		{
			...
			regrw->addr = strtoul(ptr, &ptr, 16);
			regrw->data = strtoul(ptr, &ptr, 16);

			val.len++;
		}
		val.value.regrw = regrw;
		break;
	case SWITCH_TYPE_NOVAL:
		if (str && !strcmp(str, "0"))
			return 0;
